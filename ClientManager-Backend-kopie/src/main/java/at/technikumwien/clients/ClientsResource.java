...
public class ClientsResource {
	...
	@POST
	@Transactional
	public Response create(Clients clients) {
		LOGGER.info("create >> client=" + clients);

		em.persist(clients);
		URI uri = uriInfo.getAbsolutePathBuilder().path(clients.getId().toString()).build();
	...
	@GET
	public List<Clients> retrieveAll ( @QueryParam("searchterm") String searchTerm ) {
		LOGGER.info("retrieveAll");
		
		if (searchTerm == null) {
			return clientsService.getAllClients();
		} else {
			return clientsService.getAllSearched(searchTerm);
		}
	}
}